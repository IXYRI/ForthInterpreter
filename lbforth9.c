...
int keyWaiting(void) { return positionInLineBuffer < charsInLineBuffer ? -1 : 0; }
...
BUILTIN(35, ">CFA", cfa, 0) {
	...
	if (ret < maxBuiltinAddress) push(readMem(ret));
	else push(ret);
}
...
BUILTIN(38, "QUIT", quit, 0) {
	...
	for (exitReq = 0; exitReq == 0;) {
		...
		if (address) {
			...
			if (*state && !immediate) {
				if (command < MAX_BUILTIN_ID && command != docol_id) push(command);
				else push(commandAddress);
				comma();
			}
			else {
			...
			if (notRead) {
				tell("Unknown word: ");
				for (i = 0; i < memory [0]; i++) putkey(memory [i + 1]);
				putkey('\n');
		...
		if (errorFlag) *sp = *rsp = 1;
		else if (!keyWaiting() && !(*initscript_pos)) tell(" OK\n");
	}
}
...
void addBuiltin(cell code, char const *name, byte const flags, builtin f) {
	if (errorFlag) return;
